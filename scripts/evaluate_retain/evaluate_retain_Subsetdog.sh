...
for method in "${!checkpoints[@]}"; do
    ...
    for checkpoint_path in "${ADDR[@]}"; do
        ...
        python evaluate_CLIP_retain.py -unlearning_goal $unlearn_goal -template $template -unlearn_fine_classes "${Unlearn_difficult_classes[@]}" -unlearn_dataset $dataset -unlearning_method $method -model_dir "$checkpoint_path" -unlearn
        echo "Evaluating $method using checkpoint $checkpoint_path on $dataset without unlearn activated"
        python evaluate_CLIP_retain.py -unlearning_goal $unlearn_goal -template $template -unlearn_fine_classes "${Unlearn_difficult_classes[@]}" -unlearn_dataset $dataset -unlearning_method $method -model_dir "$checkpoint_path"
    done
done
