...
checkpoints["GDiff"]="$CHECKPOINT_ROOT/Compcars/Unlearning/GDiff/fine/Cosine-Epo_6-Lr_0.00000008/difficult_KLc_20.0_KLf=20.0_Margin_0.0_beta_0.0.pth"
...
for method in "${!checkpoints[@]}"; do
    ...
    for checkpoint_path in "${ADDR[@]}"; do
        ...
        python evaluate_retain.py -unlearning_goal $unlearn_goal -template $template -unlearn_fine_classes "${Unlearn_easy_classes[@]}" -unlearn_dataset $dataset -unlearning_method $method -model_dir "$checkpoint_path" -unlearn
        echo "Evaluating $method using checkpoint $checkpoint_path on $dataset without unlearn activated"
        python evaluate_retain.py -unlearning_goal $unlearn_goal -template $template -unlearn_fine_classes "${Unlearn_easy_classes[@]}" -unlearn_dataset $dataset -unlearning_method $method -model_dir "$checkpoint_path"
    done
done
