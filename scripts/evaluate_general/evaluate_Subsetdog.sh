...
declare -a datasets=('OxfordPet') #("Stanford_Cars" "Food101" "Flower102" "caltech101" "cifar100") # "cifar100" "Stanford_Cars" "Food101" "Flower102"
declare -a methods=('KL')  #('GDiff' 'KL' 'neg' 'Hinge' 'NPO' 'SALUN' 'ME' 'Relabeling') #'Relabeling' 'KL' 'neg''KL' 'GDiff'
...
# for method in "${methods[@]}"; do
#     for dataset in "${datasets[@]}"; do
#         echo "Evaluating $method on $dataset dataset"
#         python evaluate.py -evaluate_dataset $dataset -template $dataset  -unlearn_fine_classes "${Unlearn_easy_classes[@]}" -unlearning_method $method -model_dir "${checkpoints[$method]}"
#     done
# done
